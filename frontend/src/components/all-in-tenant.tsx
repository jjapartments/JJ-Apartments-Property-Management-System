...
export function TenantDetails({ tenant, isCurrEditing }: TenantDetailsProps) {
    ...
    const handleSubmit = async () => {
        ...
        if (formData.messengerLink && !fbRegex.test(formData.messengerLink)) {
            newErrors.messengerLink = "Invalid Facebook link format.";
        }*/
        ...
        try {
            await api.patch(`/api/tenants/update/${tenant.id}`, payload);
    ...
    const confirmMoveOut = async (moveOutDate: string) => {
        try {
            const payload = { move_out_date: moveOutDate };
            await api.patch(`/api/tenants/${tenant.id}/move-out`, payload);
            ...
            console.log("Move out confirmed:", moveOutDate);

...
import { UnitHistoryModal } from "@/components/history";
...
export function ApartmentList() {
  ...
  >(null);
  const [forViewTenantData, setForViewTenantData] = useState<
    TenantWithUnitDetails[] | null
  >(null);
  ...
  const handleViewApartment = (data: TenantWithUnitDetails) => {
    console.log("Showing details for", data.firstName, "on", data.unit.name);
    setSelectedTenant(data);
    setIsViewModalOpen(true);
  };
  const handleViewHistory = (u: Unit) => {
    console.log("Showing history for", u.name);
    setSelectedUnit(u);
  ...
  const handleSave = async (updated: Unit) => {
    try {
      await api.patch(`/api/units/update/${updated.id}`, updated);
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      ...
      try {
        ...
        const tenantData = await api.get<any[]>("/api/tenants");
        const subtenantData = await api.get<any[]>("/api/subtenants");

        const processed: TenantWithUnitDetails[] = unitData.map((u: Unit) => {
          const tenant = tenantData.find((t) => t.id === u.activeTenantId);
  ...
  const fetchUnitsAndTenants = async () => {
    try {
      ...
      const unitData = await api.get<Unit[]>("/api/units");
      const tenantData = await api.get<any[]>("/api/tenants");
      const subtenants = await api.get<any[]>("/api/subtenants");
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        console.log(
          "Apartment list: Refreshing units due to trigger:",
          refreshTrigger
        );
    ...
    if (refreshTrigger > 0) {
      console.log(
        "Apartment list: refreshTrigger changed to:",
        refreshTrigger,
        "- fetching units"
      );
      fetchUnits();
                ...
                <tr key={data.unit.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {data.phoneNumber ? `${data.phoneNumber}` : "No tenant"}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    {data.subTenants.length + (data.firstName ? 1 : 0)} /{" "}
                    {data.unit.numOccupants}
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-900">
                    {formatPrice(data.unit.price)}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => handleViewHistory(data.unit)}
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-700">
              Showing <span className="font-semibold">{units.length}</span>{" "}
              entries
            </div>
          </div>
        </div>
    ...
    <div className="flex-1 bg-gray-50 p-6 overflow-auto">
      ...
      {selectedUnit && (
        <EditUnitCard
          open={editOpen}
          onClose={() => {
            setEditOpen(false), setSelectedUnit(null);
          }}
          onSave={handleSave}
          unit={selectedUnit}
        />
      ...
      {selectedTenant && (
        <AllInModal
          ...
          onClose={() => {
            setIsViewModalOpen(false), setSelectedTenant(null);
          }}
      ...
      {selectedUnit && (
        <UnitHistoryModal
          ...
          onClose={() => {
            setIsHistoryModalOpen(false), setSelectedTenant(null);
          }}
          unit={selectedUnit}
        />
      )}

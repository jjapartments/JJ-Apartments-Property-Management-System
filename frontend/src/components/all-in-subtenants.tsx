...
export function SubTenantDetails({
    ...
    const handleSubmit = async () => {
        ...
        const currentIds = new Set(
            formData.filter((st) => st.id).map((st) => st.id)
        );
        ...
        const updated = formData.filter((st) => {
            ...
            return FIELDS_TO_COMPARE.some(
                (field) =>
                    st[field as keyof Subtenant] !==
                    original[field as keyof Subtenant]
            );
        });
        ...
        try {
            ...
            for (const sub of added) {
                requests.push(
                    fetch(
                        `${process.env.NEXT_PUBLIC_API_URL}/api/subtenants/add`,
                        {
                            method: "POST",
            ...
            for (const sub of updated) {
                requests.push(
                    fetch(
                        `${process.env.NEXT_PUBLIC_API_URL}/api/subtenants/update/${sub.id}`,
                        {
                            method: "PATCH",
                            headers: { "Content-Type": "application/json" },
                            body: JSON.stringify(sub),
                        ...
                        if (!res.ok) {
                            ...
                            throw new Error(
                                err?.error ||
                                    `Failed to update subtenant ${
                                        sub.firstName || sub.id
                                    }`
                            );
                        }
                    })
            ...
            for (const sub of removed) {
                requests.push(
                    fetch(
                        `${process.env.NEXT_PUBLIC_API_URL}/api/subtenants/${sub.id}`,
                        {
                            method: "DELETE",
                        }
                    ).then(async (res) => {
    ...
    const handleChange = (index: number, field: string, value: string) => {
        ...
        const hasChanges =
            JSON.stringify(updated) !== JSON.stringify(subtenants);
        setHasUnsavedChanges(hasChanges);
        isCurrEditing?.(hasChanges);
    };

...
export function SubTenantDetails({
  subtenants,
  maxOccupants,
  isCurrEditing,
  noTenant,
  ...
  useEffect(() => {
    if (isEditing) {
      const hasChanges =
        JSON.stringify(formData) !== JSON.stringify(subtenants);
  ...
  const handleSubmit = async () => {
    ...
    const updated = formData.filter((st) => {
      ...
      return FIELDS_TO_COMPARE.some(
        (field) => st[field as keyof Subtenant] !== original[field as keyof Subtenant]
      );
    });
    ...
    try {
      // Add new subtenants
      const addRequests = added.map((sub) => api.post("/api/subtenants/add", sub));

      // Update existing subtenants
      const updateRequests = updated.map((sub) => api.patch(`/api/subtenants/update/${sub.id}`, sub));

      // Delete removed subtenants
      const deleteRequests = removed.map((sub) => api.delete(`/api/subtenants/${sub.id}`));

      await Promise.all([...addRequests, ...updateRequests, ...deleteRequests]);
      ...
      setHasUnsavedChanges(false);
      isCurrEditing?.(false);
      if (onSubmit) await onSubmit(null);
      triggerRefresh?.();
    } catch (error: unknown) {
      const displayMessage =
        error instanceof ApiError
          ...
          ? error.message
          : "Failed to save subtenant changes.";
      console.error("Subtenant update error:", error);
